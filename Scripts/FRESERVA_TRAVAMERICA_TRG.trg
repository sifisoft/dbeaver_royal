CREATE OR REPLACE TRIGGER FRESERVA_TRAVAMERICA_TRG
after insert or update ON FRESERVA for  each row
declare
    ...
    */
    if inserting then
       ...
       if :new.rv_can_f is not null  then
        ...
        -- No ingresar la reseva "caminada" a TravAmerica pues solo trabajamos con reservas originales(renta)..
        -- No se manda el movimiento para el usuario que sea..
       elsif :old.rv_fase is not null and :new.rv_fase is not null and :old.rv_fase != :new.rv_fase then
           return ;
       end if;
    end if;
    ...
    --if :new.rv_mayorista in ('AME','EUR','FIT') and trunc(sysdate) not between :new.rv_llegada and :new.rv_salida then
        return;
    end if;
