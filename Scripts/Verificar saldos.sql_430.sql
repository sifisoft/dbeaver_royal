...
select * from ccmaes where cc_tipo = 1 and cc_eje = 13
...
--<><><>  PAGOS Y PREPAGOS.... 
        ...
        -- 43001020101004
          select *
          from ccmaes a
          where cc_tipo = 1 and cc_cuenta like '%'||substr(:cta,6,7)  and cc_importe <> cc_saldo
                and cc_importe-cc_saldo <> (select sum(b.cc_importe) from ccmaes b where b.cc_emp = a.cc_emp and b.cc_plan = a.cc_plan and substr(a.cc_cuenta,6,7) = substr(b.cc_cuenta,6,7) and b.cc_tipo in (6,8) and b.cc_numero = a.cc_numero) 
          order by cc_emp, cc_plan, cc_eje, cc_per, cc_asi



select * from diag where dg_emp = '60' and dg_plan = 'A' and dg_cg = :cuenta and dg_dh = 'H'                 
              
-- Obtiene saldos de prepagos...  ojo 430's ... 
--- <><> paso 1
...
--<><> paso 3.1 checa los pagos en CCMAES, debe ser igual al paso 2   430-D
select sum(cc_importe) from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo in ('1')

-- <><> paso 3.2 checa total de prepagos en CCMAES , debe de ser igual al  paso 2  430 - H
select sum(cc_importe) from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo in ( '6','8')

-- <><> paso 3.3 checa saldo de prepagos en CCMAES , debe de ser igual a la diferencia del paso 2
select sum(cc_saldo) from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo = '1'


        -- Verifica 470-D vs.  Pagos...  (se utiliza substr pues un prepago llega a la 430.. pero los pagos van a diferentes hoteles)... 
        ...
        from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo in ('1')
        group  by cc_emp, cc_plan, cc_eje, cc_per, cc_asi

        select  cc_emp, cc_plan, cc_eje, cc_per, cc_asi, cc_importe 
        from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo  = '1'
        minus
        select dg_emp, dg_plan, dg_eje, dg_per, dg_asi, dg_imp 
        from diag where dg_emp= '60' and dg_plan = 'A' and dg_cg = :cuenta and dg_per not in ('0','99') and dg_dh = 'D'
        ...
        -- Verifica 430-H vs. Prepagos.  (aqui no se usa substr pues la cuenta es 430.. tal cual)
        select cc_emp, cc_plan, cc_eje, cc_per, cc_asi, cc_importe from ccmaes where cc_emp = '60' and cc_plan = 'A'  and  cc_cuenta = :cuenta and cc_tipo in ('6','8')
        minus
...
-- Saldos de facturas  en negativo ???
select *
from ccmaes
where cc_emp = '60' and cc_plan = 'A' and cc_eje = 13 and cc_tipo = 6 and cc_saldo < 0
...
from ccmaes a
where cc_emp = '60' and cc_plan = 'A' and cc_eje = 12 and cc_tipo = 7 
...
where   cc_emp = '60' 
        ...
        and cc_eje = 12 
        and cc_tipo = 7 
        and cc_importe <> cc_saldo
        and cc_cuenta = :cuenta
        and cc_importe-cc_saldo <> ( select sum(cc_importe) from ccmaes b where b.cc_emp = a.cc_emp and b.cc_plan = a.cc_plan and b.cc_tipo = 6 and b.cc_numero = a.cc_numero ) 
        
        + 
                                                   (select sum(dg_imp) from diag  where dg_emp = a.cc_emp and dg_plan = a.cc_plan  and substr(dg_cg,8,5) = substr(a.cc_cuenta,8,5) and dg_dh = 'D'  and dg_desc like '%'||a.cc_numero||'%'))  


select *
from diag
where dg_dh = 'D' and substr(dg_cg,8,5) = substr(:cuenta,8,5) and dg_desc like '%REF%'
