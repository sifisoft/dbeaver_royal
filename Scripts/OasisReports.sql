...
-- Avalon
execute refreshrsavailable_sum_trg_avalon(trunc(trunc(sysdate)-1,'MONTH'),last_day(add_months(trunc(sysdate),1)));

execute refreshrsavailable_trg_avalon(trunc(trunc(sysdate)-1,'MONTH'),last_day(add_months(trunc(sysdate),1)));


-- Facturacion
execute refreshrsavailable_sum_trg_inv(trunc(trunc(sysdate)-30,'MONTH'),last_day(trunc(sysdate)-21));

execute refreshrsavailable_trg_inv(trunc(trunc(sysdate)-30,'MONTH'),last_day(trunc(sysdate)-21));
...
select unique ma_hotel from rsmaes_cielo where ma_arrival > sysdate-120 and ma_arrival < sysdate;
...
-- Facturacion
execute refreshrsavailable_sum_trg_inv('01-nov-22','30-nov-22');
