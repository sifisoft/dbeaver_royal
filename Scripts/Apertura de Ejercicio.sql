...
-- A ejecutar los d�as primero de cada a�o... 
    --
    -- Modificar ci_eje y ci_lastdigit_year 
    select t.*, t.rowid
    from company t;
    ...
    insert into cuentas
    select ct_emp, ct_plan, :ejeDestino, ct_cg, ct_desc, ct_sub, ct_tcg, ct_nivel, ct_otcec, ct_fecha, ct_ctady, ct_usua, ct_operator
    from cuentas
    where ct_emp = :empresa and ct_plan = 'A' and ct_eje = :ejeOrigen;




select * from eperiod where ep_eje = 21;

-- Una vez que creados los periodos para empresa base 50, copiarlos al resto de las empresas.
insert into eperiod
select :company, ep_plan, ep_eje, ep_per, ep_fini, ep_ffin, ep_situ
from eperiod
where ep_emp = 50 and ep_plan = 'A' and ep_eje = 22;


select * from numeros where nm_eje = 21 order by nm_emp, nm_per;

-- Numeros
insert into numeros
select :destEmp, nm_plan, 21, nm_per, :Secuencia
from  numeros 
where nm_emp = :sourceEmpresa and nm_eje = 21
order by nm_emp, nm_plan, nm_eje, nm_per;

select t.*, t.rowid from numeros t where nm_emp = :sourceEmpresa and nm_eje = :newEje;

delete from numeros where nm_emp = 40 and nm_eje = 21 


select * from banoas where bo_oa_codigo = :company and bo_eje = :eje;
...
select * from cuentas where ct_emp = :company and ct_eje = 20;

delete from cuentas where ct_emp = :company and ct_eje = 20;
